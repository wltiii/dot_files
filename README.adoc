...
|copy-to-branch | git stash; git checkout $1; git stash pop |allows copying of changes to a different branch. technically a function, not an alias
|commit-distrib | git log --format=format: --name-only \| egrep -v "^$" \| sort \| uniq -c \| sort -r \| head -5 |shows files in order of number of commits
|===
